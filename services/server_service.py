...
async def create_server_config(guild_id: int, ranking_channel_id: int,
                         members_channel_id: int, admin_role_id: int) -> bool:
    ...
    try:
        await execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (guild_id, ranking_channel_id, members_channel_id, admin_role_id))
...
async def get_members_commands_channel_id(guild_id: int):
    row = await fetch_one("SELECT members_commands_channel_id FROM server_configs WHERE guild_id = ?", (guild_id,))
    if not row:
        return None
    return row[0]
