...
from ui.views.crea_evento import CreaEventoView1
...
class NomeEventoModal(discord.ui.Modal, title="Nuovo evento"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if interaction.guild is None:
            return
        event_id = await create_event(interaction.guild.id, self.name.value)
        await set_players_per_team(event_id, 3)
        await set_kill_points_db(event_id, 1)
        await insert_placement_points(event_id, list(str(val) for val in DEFAULT_PLACEMENT_POINTS.values()))
